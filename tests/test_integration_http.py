...
async def test_health(proxy_process, ports: dict):
    async with httpx.AsyncClient() as c:
        for i in range(15):
            try:
                r = await c.get(f"http://127.0.0.1:{ports['proxy']}/health", timeout=1.0)
    ...
    pytest.fail("Health endpoint not ready")
...
async def test_dashboard(proxy_process, ports: dict):
    async with httpx.AsyncClient() as c:
        for i in range(10):
            try:
                r = await c.get(f"http://127.0.0.1:{ports['proxy']}/_mcpguard/", timeout=1.0)
                if r.status_code == 200:
                ...
                pass
            await asyncio.sleep(0.5)
    pytest.fail("Dashboard not ready")


@pytest.mark.asyncio
async def test_sse_and_messages(proxy_process, ports: dict):
    """Full SSE + message flow: keep SSE alive while POSTing messages."""
    async with httpx.AsyncClient() as c:
        known_sids: list[str] = []
